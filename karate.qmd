...
Weakly consistent community detection for a given number of classes is possible under the assumption $(n-1)\rho_n\gg\log n$ [@lei2015], and the likelihood modularity is strongly consistent in the case $K=2$.

Strong consistency of the likelihood modularity for arbitrary $K$ was claimed by @bickel2009 to hold under $(n-1)\rho_n\gg\log n$. **However**, this was shown under the assumption that the modularity is globally Lipschitz.

Since this is not the case in general, it is unclear if $(n-1)\rho_n\gg\log n$ is sufficient for strong consistency. However, it is known that $(n-1)\rho_n\gg(\log n)^2$ is sufficient for the likelihood (and thus also the Bayesian) modularity to be strongly consistent.
